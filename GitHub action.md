...
1. docker compose คืออะไร มีความสำคัญอย่างไร
  : คือเครื่องมือที่ใช้กำหนดและรันหลายๆ container เป็นชุดเดียวด้วยไฟล์ YAML เพื่อจัดการ services, networks, volumes และการเชื่อมต่อระหว่างกัน
2. GitHub pipeline คืออะไร เกี่ยวข้องกับ CI/CD อย่างไร
  :ระบบ automation ที่รัน workflows บนเหตุการณ์ (push/PR) เพื่อทำ CI/CD เช่น ทดสอบ สแกน และ build/deploy อัตโนมัติ
3. จากไฟล์ docker compose  ส่วนของ volumes networks และ healthcheck มีความสำคัญอย่างไร
  :เก็บข้อมูลถาวรหรือ mount โค้ดจาก host ช่วยให้ข้อมูลไม่หายเมื่อ container หยุดและรองรับการพัฒนาแบบ live-reload
    ...
    services:
      postgres:
        ...
        options: >-
          ...
          --health-retries 5
```
  :โค้ด yaml ส่วนนี้เป็นการกำหนดงานชื่อ test ที่ใช้สำหรับการรันการทดสอบ โดยให้ทำงานบนเครื่องเสมือนที่ใช้ระบบปฏิบัติการ ubuntu-latest และมีการเปิดใช้งานฐานข้อมูล Postgres ผ่าน container โดยใช้ image postgres:16-alpine ซึ่งถูกกำหนดค่าเริ่มต้นไว้ เช่น ชื่อผู้ใช้ รหัสผ่าน และชื่อฐานข้อมูลที่ต้องสร้างขึ้น พร้อมทั้งมีการเปิดพอร์ต 5432 เพื่อให้เข้าถึงฐานข้อมูลได้ นอกจากนี้ยังมีการตั้งค่า health check โดยใช้คำสั่ง pg_isready เพื่อเช็กว่าฐานข้อมูลพร้อมทำงานจริงหรือยัง ก่อนที่จะเริ่มขั้นตอนการทดสอบ
    ...
    steps:
      ...
      - name: Set up Python
        ...
        with:
          ...
          cache: 'pip'
```
  :การใช้ actions/checkout@v4 เป็นขั้นตอนที่เอาไว้ดึงซอร์สโค้ดจาก GitHub repository มาที่ runner เพื่อให้ workflow สามารถนำโค้ดมาใช้งานได้ ส่วน actions/setup-python@v5 เป็นการติดตั้งและตั้งค่า Python บน runner โดยเราสามารถเลือกเวอร์ชันที่ต้องการใช้ และยังสามารถเปิด cache ของ pip เพื่อช่วยให้การติดตั้ง library เร็วขึ้นเพราะไม่ต้องโหลดใหม่ทุกครั้ง
6. Snyk คืออะไร มีความสามารถอย่างไรบ้าง
  :เครื่องมือที่ใช้ตรวจสอบช่องโหว่ด้านความปลอดภัยในโค้ดและส่วนประกอบต่าง ๆ ของระบบ ไม่ว่าจะเป็น dependencies, container images หรือไฟล์ Infrastructure as Code โดยมันสามารถบอกได้ว่ามีช่องโหว่อะไร และแนะนำวิธีแก้ไข เช่น อัปเดตไปใช้เวอร์ชันที่ปลอดภัยกว่า นอกจากนี้ Snyk ยังสามารถเชื่อมต่อกับ CI/CD pipeline ได้ ทำให้ทุกครั้งที่มีการ build หรือ deploy ระบบจะถูกตรวจสอบความปลอดภัยโดยอัตโนมัติ